...
4. Build the save.
   ...
   - Set `isCommander: true` only for actual commanders that start outside the library. Set `isAlternateCommander: true` only for explicitly identified possible commanders that remain in the library; otherwise set it to false.
